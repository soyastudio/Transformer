...
@Command(name = "bod-esql-template", uri = "bod://esql-template")
public class EsqlTemplateCommand extends ConstructCommand {
    ...
    private void printSimpleArray(KnowledgeTreeNode<XsNode> node, CodeBuilder builder, int indent) {
        ...
        String arrIndex = "_" + CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, nameBase).toLowerCase() + "_index";

        builder.append("-- LOOP ", indent).append(assignment.getAssign()).append(" TO ").append(node.getPath()).appendLine();
        builder.append("DECLARE ", indent).append(arrVar).append(" INTEGER 1").appendLine(";");
        builder.append("DECLARE ", indent).append(arrIndex).append(" INTEGER 1").appendLine(";");

        builder.append(arrName, indent).append(" : WHILE LASTMOVE(").append(arrVar).appendLine(") DO").appendLine();

        builder.append("-- ", indent + 1).appendLine(node.getPath());
        builder.append("CREATE LASTCHILD OF ", indent + 1).append(parent.getVariable()).append(" AS ").append(" TYPE XMLNSC.Folder NAME '").append(name).appendLine("';");
